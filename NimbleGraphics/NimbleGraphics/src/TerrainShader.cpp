...
TerrainShader::TerrainShader(TerrainShader && other)
	: IShader(std::move(other))
{
	LOG_INFO("Terrain Shader Move!");
	*this = std::move(other);
}

TerrainShader & TerrainShader::operator=(TerrainShader && other)
{
	LOG_INFO("Terrain Shader op=!");
...
TerrainShader::TerrainShader(ComPtr<ID3D11Device> device)
	: IShader(), _lightBuffer(), _sampleState()
{
...
bool TerrainShader::Draw(ComPtr<ID3D11DeviceContext> deviceContext, int indexCount, Matrix worldMatrix, 
...
bool TerrainShader::SetShaderParameters(ComPtr<ID3D11DeviceContext> deviceContext, shared_ptr<Light> light, ComPtr<ID3D11ShaderResourceView> grass, ComPtr<ID3D11ShaderResourceView> slope,
