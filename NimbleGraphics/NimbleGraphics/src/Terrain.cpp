...
//
//Terrain::Terrain()
//    : _width(0), _height(0), _numChunksX(0), _numChunksZ(0), _vertexCount(0)
//{
//}
//
//Terrain::Terrain(D3DDevice device, D3DDeviceContext deviceContext,
//                 unsigned numChunksX, unsigned numChunksZ, vector<wstring>& texture_paths)
...
//}
//
//void Terrain::Load(ComPtr<ID3D11Device> device,
...
//}
//
//void Terrain::Draw(ComPtr<ID3D11Device> device,
//                   ComPtr<ID3D11DeviceContext> deviceContext,
//                   const Matrix& viewMatrix, const Matrix& projectionMatrix,
//                   const Light& light, const Frustum& frustum)
//{
//    auto terrain_shader =
//        ShaderManager::GetInstance().GetShader<TerrainShader>(SHADER::TERRAIN);
//
//    terrain_shader->SetViewMatrix(viewMatrix);
//    terrain_shader->SetProjectionMatrix(projectionMatrix);
//    terrain_shader->SetShaderParameters();
//
//    deviceContext->OMSetBlendState(
//        StatesHelper::GetInstance().GetStates()->Opaque(), Black, 0xffffffff);
//
//    deviceContext->OMSetDepthStencilState(
//        StatesHelper::GetInstance().GetStates()->DepthDefault(), 0);
//
...
//}
//
//void Terrain::SmoothHeightmapAdd(Vector3 location, float radius, float intensity,
//                                 ComPtr<ID3D11DeviceContext> deviceContext,
...
//}
//
//void Terrain::SplatTexture(D3DDevice device, D3DDeviceContext deviceContext,
//                           Vector3 location, float radius, int intensity,
//                           int texture_number)
//{
//    // To splat a texture, we just access the splat map and draw the correct
//    // color.
...
//    Vector2 coords_pixel_space =
//        Vector2((location.x / terrain_width) * width,
...
//
//    deviceContext->Map(copyTexture.Get(), 0, D3D11_MAP_READ_WRITE, 0,
//                       &copyResourceData);
//
//    Vector2 current_loc;
//
...
//                continue;
//
//            current_loc = Vector2(((float)i / (float)width) * terrain_width,
//                                  ((float)j / (float)height) * terrain_height);
...
//
//    for (int i = 0; i < _mem_heightmap->GetHeight() * _mem_heightmap->GetWidth();
//            ++i)
//    {
...
//}
//
//void Terrain::SetWireframeRender(ComPtr<ID3D11Device> device,
//                                 ComPtr<ID3D11DeviceContext> deviceContext) const
//{
//    HRESULT result;
//
...
//}
//
//void Terrain::SetShadedRender(ComPtr<ID3D11Device> device,
//                              ComPtr<ID3D11DeviceContext> deviceContext) const
//{
//    ID3D11RasterizerState* rState;
//    D3D11_RASTERIZER_DESC rDesc;
...
//                    auto map_index =
//                        _mem_heightmap->GetIndex(z, x, chunk_width, chunk_height, j, i);
//
//                    vertices[vert_index] = p_map_vertices[map_index];
//                }
//            }
//
//            _chunks.emplace_back(x, z, chunk_width, chunk_height, device, vertices,
//                                 indices);
...
//}
//
//void Terrain::UpdateChunks(ComPtr<ID3D11Device> device,
//                           ComPtr<ID3D11DeviceContext> deviceContext)
//{
//    for (auto z = 0; z < _numChunksZ; ++z)
//    {
