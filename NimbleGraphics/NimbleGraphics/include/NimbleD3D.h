...
{
public:
	NimbleD3D(): m_vsync_enabled(false), m_videoCardMemory(0), m_hwnd(nullptr)
	{
		fieldOfView = XM_PIDIV4;
		m_projectionMatrix = std::make_shared<Matrix>();
		m_worldMatrix = std::make_shared<Matrix>();
	}
	...
	shared_ptr<Matrix> GetProjectionMatrix() const
	{
		return m_projectionMatrix;
	}

	shared_ptr<Matrix> GetWorldMatrix() const
	{
	...
	void UpdateProjectionMatrix(int width, int height, float screenNear, float screenFar) const
	{
